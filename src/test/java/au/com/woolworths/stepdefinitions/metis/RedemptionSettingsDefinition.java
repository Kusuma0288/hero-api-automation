...
public class RedemptionSettingsDefinition extends RedemptionSettingsHelper {
    ...
    public void userShouldBeAbleToChooseHowIsRewarded() {
        ...
        Assert.assertEquals("RedemptionSettings does not return the expected number of items ", 3, redemptionSettingsResponse.getRewardsRedemptionSettings().getItems().length);
        ...
        Assert.assertEquals("Incorrect confirmation message for switching to item 1", "Get $10 Woolworths Dollars when you reach 2000 points, to save or spend when you like.", redemptionSettingsResponse.getRewardsRedemptionSettings().getItems()[0].getConfirmationMessages()[0].getMessage());
